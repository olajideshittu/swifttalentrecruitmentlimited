...
class RecommendationService:
    def recommend_products(self, query, with_confidence=False):
        # Show all similar products for CNN/Product Image Upload (OR logic)
        products = []
        response = ""
        ...
        # Example specs for computers and laptops
        computer_specs = [
        ...
        def make_product(filename, default_name, default_desc, default_price, default_country, default_size, default_color, confidence=None):
            return {
                "image": filename,
                "name": default_name,
        ...
        # Special handling for "computer accessories"
        is_computer_accessories = (
        ...
        if with_confidence and query_words:
            # CNN/Product Image Upload: OR logic, match any product with at least one word in filename
            for f in files:
                fname = os.path.splitext(f)[0].replace("_", " ").replace("-", " ").lower()
        ...
        # Determine category for response
        ...
        if matched_files:
            # For CNN: best match gets highest confidence, others get lower
            confidence_map = {}
            if with_confidence and query_words:
                # Find the best match (most words in common with query)
                def match_score(fname):
            ...
            # Category-specific, professional response
